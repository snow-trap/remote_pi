...
function safePreviewPath(dir, messageId, index) {
    return join(dir, `${safeFilenameToken(messageId)}-${index}.preview.png`);
}
function cleanupPreviewFile(previewPath) {
    try {
...
async function renderablePngPathFromImage(imageData, mime, previewPath) {
...
export function decodeImagePayload(data, mime) {
...
export async function collectReceivedImagePreviews(msg) {
    ...
    for (let i = 0; i < msg.images.length; i += 1) {
        ...
        try {
            ...
            const previewPath = image.mime === IMAGE_PREVIEW_MIME
                ? undefined
                : await renderablePngPathFromImage(image.data, image.mime, safePreviewPath(dir, msg.id, i));
            previews.push({
                messageId: msg.id,
                index: i,
                mime: image.mime,
...
export function contentFromUserMessage(msg) {
...
export function registerReceivedImageRenderer(pi) {
...
export function isReceivedImageContextMessage(message) {
...
export function filterReceivedImageMessagesFromContext(messages) {
