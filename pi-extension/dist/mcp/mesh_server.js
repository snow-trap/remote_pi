...
mcp.registerTool("list_peers", {
    ...
    try {
        ...
        return { content: [{ type: "text", text: peers.length > 0 ? peers.join("\n") : "(no peers)" }] };
    }
    catch (e) {
        return { content: [{ type: "text", text: `list_peers failed: ${String(e)}` }], isError: true };
    }
});
mcp.registerTool("agent_send", {
    ...
    if (to === mesh.address() || to === mesh.name()) {
        return { content: [{ type: "text", text: "Cannot send to yourself" }], isError: true };
    }
    try {
        if (to === "broadcast") {
        ...
        return { content: [{ type: "text", text: `send failed: ${String(e)}` }], isError: true };
    }
});
mcp.registerTool("get_messages", {
    ...
    const lines = msgs.map((m) => `[${m.at}] from=${m.from}${m.re ? ` re=${m.re}` : ""}\nid=${m.id}\n${JSON.stringify(m.body, null, 2)}`);
