 ...
 */
export function decodeEd25519PublicKey(raw, field = "public key") {
    ...
    if (bytes.length !== ED25519_PUBLIC_KEY_BYTES) {
        throw new MeshPublicKeyError(field, `wrong length (${bytes.length}, expected ${ED25519_PUBLIC_KEY_BYTES})`);
    }
...
export function encodeEd25519PublicKey(bytes, field = "public key") {
    if (!(bytes instanceof Uint8Array) || bytes.length !== ED25519_PUBLIC_KEY_BYTES) {
        ...
        throw new MeshPublicKeyError(field, `wrong length (${length}, expected ${ED25519_PUBLIC_KEY_BYTES})`);
    }
    return Buffer.from(bytes).toString("base64");
}
/** Decodes then returns an Ed25519 public key in canonical standard base64. */
export function canonicalizeEd25519PublicKey(raw, field = "public key") {
...
export function publicKeyFingerprint(bytes) {
    return createHash("sha256").update(bytes).digest("hex").slice(0, 8);
}
function isRoutingAliasSafeByte(byte) {
...
export function encodeRoutingAlias(rawNickname) {
    let encoded = "";
...
export function selectRoutingNickname(candidates) {
    let selected;
    let selectedEncoded;
 ...
 */
export function allocateRoutingAliases(inputs) {
    const preparedByKey = new Map();
    for (const [index, input] of inputs.entries()) {
        const keyBytes = decodeEd25519PublicKey(input.pcPubkey, `routingAliases[${index}].pcPubkey`);
    ...
    const prepared = [...preparedByKey.values()].sort((left, right) => left.pcPubkey < right.pcPubkey
        ? -1
    ...
    for (const identity of prepared) {
        ...
        if (group)
            ...
            groupsByBase.set(identity.base, [identity]);
    }
    ...
    for (const [base, group] of collidingGroups) {
        ...
        for (let prefixLength = 8; prefixLength <= fullPrefixLength; prefixLength++) {
            const candidates = group.map((identity) => `${base}~${identity.keyUrl.slice(0, prefixLength)}`);
            const uniqueCandidates = new Set(candidates);
            const conflicts = candidates.some((candidate) => reservedBases.has(candidate) || allocatedAliases.has(candidate));
            if (uniqueCandidates.size !== group.length || conflicts)
                continue;
    ...
    return new Map(prepared.map((identity) => [
        identity.pcPubkey,
