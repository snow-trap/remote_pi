...
import { addPeer, getOrCreateEd25519Keypair, KeyringUnavailableError, listPeers, removePeer, snapshotOwnerPubkeys, conditionalRemovePeer, } from "./pairing/storage.js";
...
import { kDefaultRelayUrl, loadConfig, resolveRelayUrl, saveConfig, isValidRelayUrl, isWebSocketScheme, toWebSocketUrl, } from "./config.js";
...
function _filterReceivedImageMessagesFromContext(messages) {
...
const extension = (pi) => {
    ...
    pi.on("session_before_compact", (event) => {
        if (event.preparation) {
            event.preparation.messagesToSummarize = _filterReceivedImageMessagesFromContext(event.preparation.messagesToSummarize);
            event.preparation.turnPrefixMessages = _filterReceivedImageMessagesFromContext(event.preparation.turnPrefixMessages);
        }
...
export function _routeClientMessageFrom(sender, msg, ctx) {
    ...
    switch (msg.type) {
        ...
        case "model_set":
            void handleModelSet(_pi, (_lastEventCtx ?? _lastCtx), ensureModelRegistry(), sender, msg, _persistModelDefault);
            break;
        case "thinking_set":
            handleThinkingSet(_pi, sender, msg);
            break;
        case "list_models":
            handleListModels((_lastEventCtx ?? _lastCtx), ensureModelRegistry(), sender, msg);
            break;
    }
}
/**
