...
let _cached: ActionModelRegistry | null = null;

/** Feed the cache with the live registry from a full ExtensionContext. */
export function observeModelRegistry(reg: ActionModelRegistry | null | undefined): void {
  if (reg) _cached = reg;
}
...
export function ensureModelRegistry(): ActionModelRegistry {
  if (!_cached) throw new Error("Model registry unavailable (no session ctx yet)");
  return _cached;
}
