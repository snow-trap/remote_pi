...
import { SettingsManager, type ExtensionAPI, type ExtensionContext, type ExtensionCommandContext } from "@earendil-works/pi-coding-agent";
...
import {
  collectReceivedImagePreviews,
  contentFromUserMessage,
  REMOTE_PI_RECEIVED_IMAGE_TYPE,
  type ClientUserMessage,
  type ReceivedImageDetails,
} from "./images.js";
import { mapAgentMessagesToEvents, stringifyContent, stringifyToolResult, type BufferMsg } from "./history.js";
...
type PreviewDelivery = "immediate" | "defer";
...
export class RelayService {
  private readonly deps: RelayServiceDeps;

  private state: RelayState = "idle";
  private relay: RelayClient | null = null;
  private relayUrl: string | null = null;  // URL used by current connection

  /** Owners currently connected via the relay. Key = app peer pubkey (Ed25519,
   *  base64 standard); value = the dedicated PlainPeerChannel. */
  private readonly activePeers = new Map<string, PlainPeerChannel>();
  private peerShort = "";  // shortid of the most recently attached peer (UX hint)

  private myRoomId: string | null = null;
  private myRoomMeta: { name: string; cwd: string; model?: string; thinking?: ThinkingLevel; working?: boolean } | null = null;
  ...
  private selfRevokeEpoch = 0;
  private selfRevokeTopologyReadyEpoch = -1;
  private selfRevokeTopology: MeshTopologySnapshot | null = null;

  // Session-sync mirror state
  private sessionStartedAt: number | null = null;
  private messageBuffer: BufferMsg[] = [];
  private pendingSteers: PendingSteer[] = [];
  private lastConsumedSteerText: string | null = null;
  private queuedItems: AndroidQueuedItem[] = [];
  private currentTurnId: string | null = null;

  // Image previews pending while a turn runs
  private pendingReceivedImagePreviews: ReceivedImageDetails[] = [];

  // Cached state of global pairings (peers.json) for the footer
  private hasGlobalPairings = false;

  private readonly harness = { name: "Pi coding agent", version: readExtensionVersion() } as const;
  ...
  get isStarted(): boolean { return this.state !== "idle"; }
  get activePeerCount(): number { return this.activePeers.size; }
  get connectedPeerShort(): string { return this.peerShort; }
  get hasAnyPeer(): boolean { return this.activePeers.size > 0; }
  get pairedBefore(): boolean { return this.hasGlobalPairings; }
  get currentRelayUrl(): string | null { return this.relayUrl; }
  ...
  async start(ctx: Pick<ExtensionContext, "ui" | "cwd">): Promise<void> {
    if (this.state !== "idle") {
        ...
        if (live) {
          ...
          if (modelId) {
            const found = provider
              ? (ctx as Partial<ExtensionContext>).modelRegistry?.find(provider, modelId)
              : undefined;
    ...
    try {
      this.currentThinking = this.deps.getPi()?.getThinkingLevel() as ThinkingLevel | undefined;
    ...
    if (this.currentThinking) roomMeta.thinking = this.currentThinking;
    ...
    if (this.selfRevoke === null) {
      ...
      producer = new SelfRevoke({
        ...
        onRevoke: (rawOwnerPubkey, canonicalOwnerPubkey) => {
          if (this.selfRevoke !== producer || producerEpoch !== this.selfRevokeEpoch) return;
    ...
    if (!createdProducer) this.attachBridgeIfReady();
  ...
  stop(byeReason?: ByeReason): void {
    ...
    if (this.queuedItems.length > 0) this.resetQueuedItems({ broadcast: true });
  ...
  attachBridgeIfReady(): void {
    ...
    if (!meshNode || !this.relay || !this.relayUrl || !this.cachedEd25519) return;
    ...
    if (this.selfRevoke !== null) {
      if (
        this.selfRevokeTopologyReadyEpoch !== this.selfRevokeEpoch ||
        this.selfRevokeTopology === null
      ) {
        return;
      }
      if (!meshNode.hasTopology()) meshNode.setTopology(this.selfRevokeTopology);
    }
    void meshNode
  ...
  private onRelayClose(closedRelay: RelayClient): void {
    if (this.relay !== closedRelay) return; // delayed close from a replaced relay
    ...
    if (reconnectUrl) this.scheduleReconnect(this.lifecycleGeneration, reconnectUrl);
  }

  private isCurrentReconnect(lifecycleGeneration: number, url: string): boolean {
    return (
      lifecycleGeneration === this.lifecycleGeneration &&
      this.state === "started" &&
      this.relay === null &&
  ...
  private scheduleReconnect(lifecycleGeneration: number, url: string): void {
    if (this.reconnectTimer !== null) return;
    if (!this.cachedEd25519) return;
    if (!this.isCurrentReconnect(lifecycleGeneration, url)) return;
  ...
  private async attemptReconnect(lifecycleGeneration: number, url: string): Promise<void> {
    if (!this.cachedEd25519) return;
    if (!this.isCurrentReconnect(lifecycleGeneration, url)) return;
  ...
  async pair(ctx: Pick<ExtensionContext, "ui" | "cwd">, args = ""): Promise<void> {
    const cwd = "cwd" in ctx ? (ctx as ExtensionCommandContext).cwd : process.cwd();
    ...
    if (this.state === "idle" || !this.relay) {
      ctx.ui.notify(
        "[remote-pi] Pair requires the relay to be connected. Run /remote-pi start relay first.",
        "warning",
      );
      return;
    }
    ...
    if (pi) {
      try {
        pi.appendEntry<PairCodeEntry>("remote-pi:pair-code", {
  ...
  async revoke(arg: string, ctx: Pick<ExtensionContext, "ui" | "cwd">): Promise<void> {
    const shortid = arg.trim();
    if (!shortid) {
      ctx.ui.notify(
        "[remote-pi] Usage: /remote-pi revoke <shortid>. Run /remote-pi devices to see shortids.",
        "warning",
      );
      return;
    }
    ...
    if (this.state === "idle" || !this.relay) {
      ctx.ui.notify(
        "[remote-pi] Revoke requires the relay to be connected. Run /remote-pi start relay first.",
        "warning",
      );
      return;
    }
    ...
    if (matches.length > 1) {
      ...
      ctx.ui.notify(
        `[remote-pi] Ambiguous shortid — ${matches.length} matches: ${collisions}. Use more chars.`,
        "warning",
      );
      return;
    }
  ...
  async shortidCompletions(prefix: string): Promise<Array<{ value: string; label: string }>> {
    const peers = await listPeers().catch(() => [] as PeerRecord[]);
    return peers
      .map(inspectPeerRecord)
      .filter((peer): peer is InspectedPeerRecord => peer !== null)
  ...
  private attachPeerChannel(appPeerId: string, channel: PlainPeerChannel): void {
    this.activePeers.set(appPeerId, channel);
    this.peerShort = appPeerId.slice(0, 8);
  }

  private detachPeerChannel(appPeerId: string): void {
  ...
  onPeerDisconnect(appPeerId?: string): void {
    ...
    if (!target) return;
    if (!this.activePeers.has(target)) return;
  ...
  private attachOwner(
    ...
    if (this.activePeers.has(appPeerId)) this.detachPeerChannel(appPeerId);

    const channel = new PlainPeerChannel(
      relay,
      appPeerId,
      this.myRoomId ?? undefined,
      (msg) => this.routeClientMessageFrom(channel, msg, this.liveCtx() ?? NOOP_CTX),
      () => this.onPeerDisconnect(appPeerId),
    );
    ...
    this.deps.notify(
      ...
      `(${this.activePeers.size} active)`,
      "info",
    );
    return channel;
  }

  private liveCtx(): Pick<ExtensionContext, "ui" | "abort" | "cwd"> | null {
    return this.deps.getEventCtx() as Pick<ExtensionContext, "ui" | "abort" | "cwd"> | null
      ?? this.deps.getCommandCtx();
  }

  // ── Auto-listener ─────────────────────────────────────────────────────────

  private installAutoListener(relay: RelayClient): () => void {
    ...
    const onMsg = async (line: string) => {
      ...
      if (this.activePeers.has(outer.peer)) return;
  ...
  private async handlePairRequest(
    ...
    const cwd = commandCtx && "cwd" in commandCtx
      ? (commandCtx as ExtensionCommandContext).cwd
      : process.cwd();
    const sessionName = this.deps.getDisplayName(cwd);
  ...
  private async findKnownPeer(appPeerIdStd: string): Promise<PeerRecord | null> {
  ...
  private revokeActiveOwnerRuntime(canonicalOwnerPubkey: string): void {
    if (!this.activePeers.has(canonicalOwnerPubkey)) return;
    this.refreshPairingsCache();
  ...
  routeClientMessageFrom(
    ...
    switch (msg.type) {
      ...
      case "user_message": {
        ...
        if (seededTurnId) this.currentTurnId = msg.id;
        ...
        if (!wake.ok) {
          if (seededTurnId) this.currentTurnId = previousTurnId;
          sender.send({
            type: "error",
            code: "internal_error",
            in_reply_to: msg.id,
        ...
        if (shouldSteer) this.trackPendingSteer(msg.id, msg.text);
      ...
      case "session_compact":
        handleSessionCompact((this.deps.getEventCtx() ?? this.deps.getCommandCtx()) as ActionCtx | null, sender, msg);
        break;
      case "session_new": {
        const actionCtx = this.deps.getCommandCtx() as ActionCtx | null;
        ...
        void handleSessionNew(actionCtx, sender, msg, (freshCtx) => setCommandCtx(freshCtx))
          .then((created) => {
            ...
            if (created) this.resetSessionForNew(msg.id);
          });
        break;
      }
      case "model_set": {
        const actCtx = (this.deps.getEventCtx() ?? this.deps.getCommandCtx()) as ActionCtx | null;
        observeModelRegistry(actCtx?.modelRegistry);
        let reg: ReturnType<typeof ensureModelRegistry>;
        try {
  ...
  private abortCurrentTurn(fallbackCtx?: Pick<ExtensionContext, "abort">): boolean {
    ...
    for (const candidate of candidates) {
      if (!candidate || candidate === NOOP_CTX) continue;
  ...
  private resetSessionForNew(inReplyTo: string): void {
    this.messageBuffer = [];
    this.pendingSteers = [];
    this.lastConsumedSteerText = null;
  ...
  private queuedStateMessage(): ServerMessage {
  ...
  private sendQueuedState(sender: PlainPeerChannel): void {
    sender.send(this.queuedStateMessage());
  }

  private broadcastQueuedState(): void {
    this.broadcastToActive(this.queuedStateMessage());
  }

  private resetQueuedItems({ broadcast = false }: { broadcast?: boolean } = {}): void {
    this.queuedItems = [];
    if (broadcast) this.broadcastQueuedState();
  }

  private upsertQueuedItem(item: AndroidQueuedItem): void {
  ...
  private clearQueuedItems(targetId?: string): void {
  ...
  private isBusyForQueueDrain(): boolean {
  ...
  maybeDrainQueuedItem(): void {
    if (this.isBusyForQueueDrain()) return;
  ...
  private trackPendingSteer(id: string, text: string): void {
    const key = normalizeSteerText(text);
    if (!key) return;
    this.pendingSteers.push({ id, text: key });
  }

  private consumePendingSteerForStartedUser(text: string): string | null {
  ...
  private broadcastConsumedSteerForUserContent(content: unknown): void {
  ...
  private echoUserMessage(msg: ClientUserMessage, forceSteer = false): void {
    this.broadcastToActive({
      type: "user_message",
      id: msg.id,
      text: msg.text,
  ...
  private shouldDeferReceivedImagePreview(): boolean {
    return this.currentTurnId !== null || this.myRoomMeta?.working === true;
  }

  private sendReceivedImagePreviewNow(details: ReceivedImageDetails): void {
    const pi = this.deps.getPi();
    if (!pi) return;
    try {
      pi.sendMessage<ReceivedImageDetails>({
  ...
  private flushPendingReceivedImagePreviews(): void {
    if (this.pendingReceivedImagePreviews.length === 0) return;
    const pending = this.pendingReceivedImagePreviews.splice(0);
    for (const details of pending) this.sendReceivedImagePreviewNow(details);
  }

  private async emitReceivedImagePreviews(
    msg: ClientUserMessage,
    delivery: PreviewDelivery = "immediate",
  ): Promise<void> {
  ...
  private async deliverImageUserMessage(
    sender: PlainPeerChannel,
    msg: ClientUserMessage,
    shouldSteer: boolean,
  ): Promise<void> {
    const previewDelivery: PreviewDelivery =
      shouldSteer || this.currentTurnId !== null || this.myRoomMeta?.working === true
        ? "defer"
        : "immediate";
    ...
    if (previewDelivery === "immediate") {
      ...
      void emitPreview().finally(() => {
        if (!this.shouldDeferReceivedImagePreview()) this.flushPendingReceivedImagePreviews();
      });
    }
  ...
  onUserInput(event: { text: string; source?: string }): void {
    if (!this.hasAnyPeer) return;
  ...
  onModelSelect(modelName: string): void {
    ...
    if (this.myRoomMeta) this.myRoomMeta = { ...this.myRoomMeta, model: modelName };
  ...
  onThinkingSelect(level: ThinkingLevel): void {
    ...
    if (this.myRoomMeta) this.myRoomMeta = { ...this.myRoomMeta, thinking: level };
    if (this.relay && this.myRoomId) {
  ...
  onTurnStart(ctx: unknown): void {
    if (!this.currentModel) {
      try {
        const m = (ctx as { getModel?: () => { name?: string; id?: string } | undefined }).getModel?.();
        const name = m?.name ?? m?.id;
        if (name) this.onModelSelect(name);
  ...
  private publishWorking(working: boolean): void {
    if (this.myRoomMeta) this.myRoomMeta = { ...this.myRoomMeta, working };
    if (this.relay && this.myRoomId) {
  ...
  onMessageStart(event: { message?: unknown }): void {
    ...
    if (!this.hasAnyPeer || message?.role !== "user") return;
    this.broadcastConsumedSteerForUserContent(message.content);
  }

  onMessageUpdate(event: { assistantMessageEvent: { type: string; delta?: string } }): void {
    if (!this.hasAnyPeer || !this.currentTurnId) return;
  ...
  onToolExecutionStart(event: { toolCallId: string; toolName: string; args: unknown }): void {
    if (!this.hasAnyPeer) return;
    this.broadcastToActive({
      type: "tool_request",
  ...
  onToolExecutionEnd(event: { toolCallId: string; result: unknown; isError: boolean }): void {
    if (!this.hasAnyPeer) return;
  ...
  onMessageEnd(event: { message?: unknown }): void {
    ...
    if (m.role === "user" || m.role === "assistant" || m.role === "toolResult") {
      this.messageBuffer.push(m as unknown as BufferMsg);
    }
    ...
    if (m.role === "assistant" && m.stopReason === "error" && this.hasAnyPeer) {
      const message = typeof m.errorMessage === "string" && m.errorMessage
        ? m.errorMessage
        : "Provider error";
      const errMsg: ServerMessage = this.currentTurnId
  ...
  onCompact(event: { compactionEntry?: unknown }): void {
...
function rawOwnerFingerprint(rawValue: unknown): string {
...
function runtimeOwnerFingerprint(runtimeKey: string): string {
  try {
...
function inspectPeerRecord(record: unknown): InspectedPeerRecord | null {
 ...
 */
function reportRevocationByFingerprint(pi: ExtensionAPI | null, canonicalOwnerPubkey: string): void {
  if (!pi) return;
 ...
 */
function persistModelDefault(provider: string, modelId: string): void {
  try {
