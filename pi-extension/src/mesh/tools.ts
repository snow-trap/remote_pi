...
const NOT_IN_SESSION = "Not in a session. Run /remote-pi start mesh first";
 ...
 *   - `list_peers`     — current peer inventory as opaque addresses.
 *
 * Reply pattern: when you receive a message you want to answer, send back
 * another envelope with `re=<original-id>`. The original sender sees that
 * reply in its inbox during a future turn. (The deprecated `agent_request`
 * blocking tool was removed in the lean rewrite.)
 *
 * `getSessionPeer` is a getter (not a captured value) so changes to the
 * underlying node are observed live.
 */
export function registerAgentTools(
  ...
  pi.registerTool<typeof ListPeersParams, { peers: string[] }>({
    ...
    description:
      ...
      "opaque routing key — pass it to `agent_send` VERBATIM, " +
