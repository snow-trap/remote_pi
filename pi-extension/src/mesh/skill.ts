...
function packagedSkillPath(): string | null {
  ...
  if (existsSync(src1)) return src1;
  if (existsSync(src2)) return src2;
  return null;
}

/** Copy the packaged skill into the discovered skills dir. Best-effort. */
export function deployAgentNetworkSkill(): void {
...
export function undeployAgentNetworkSkill(): void {
  const dstDir = join(skillsDir(), "agent-network");
  try {
