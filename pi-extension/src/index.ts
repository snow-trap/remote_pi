...
import { isPrintMode, resolveFeatureFlags, type FeatureFlags } from "./flags.js";
import { RelayService, type PairCodeEntry } from "./relay/relay_service.js";
...
import { ensureGlobalDirs, skillsDir, LOCAL_SESSION_NAME } from "./mesh/paths.js";
...
const extension: ExtensionFactory = (pi: ExtensionAPI): void => {
  ...
  let lastCtx: Pick<ExtensionContext, "ui" | "abort" | "cwd"> | null = null;
  let lastEventCtx: (Pick<ExtensionContext, "compact" | "abort" | "ui" | "cwd"> & { getModel?: () => { name?: string; id?: string } | undefined }) | null = null;
  let disposed = false;
  let autoInited = false;

  const flags: FeatureFlags = resolveFeatureFlags((name) => {
    try { return pi.getFlag(name); } catch { return undefined; }
  });
  ...
  function liveCtx(): Pick<ExtensionContext, "ui"> | null {
    return lastEventCtx ?? lastCtx;
  }

  function safeNotify(message: string, level: "info" | "warning" | "error" = "info"): void {
    try {
      const ui = liveCtx()?.ui as { notify?: (m: string, l?: string) => void } | undefined;
      ui?.notify?.(message, level);
    } catch { /* stale ctx — never let notify take down the process */ }
  }

  function refreshFooter(): void {
    let ui: { setStatus?: (k: string, v: string | undefined) => void; setTitle?: (t: string) => void } | null;
    try {
      ui = (liveCtx()?.ui as typeof ui) ?? null;
    } catch {
      return;
    }
  ...
  function displayName(cwd: string): string {
    return mesh.currentNode?.name()
      ?? sanitizeSegment(safeGetSessionName())
      ?? defaultAgentName(cwd);
  }

  function safeGetSessionName(): string | undefined {
  ...
  const relay = new RelayService({
    ...
    setCommandCtx: (ctx) => { lastCtx = ctx as typeof lastCtx; },
  ...
  function enableMeshSurface(): void {
  ...
  try {
    ensureGlobalDirs();
    if (flags.mesh) enableMeshSurface();
    else undeployAgentNetworkSkill();
  ...
  pi.registerEntryRenderer<PairCodeEntry>("remote-pi:pair-code", (entry, _options, theme) => {
  ...
  pi.registerEntryRenderer<{ text: string }>("remote-pi:mesh-revoked", (entry, _options, theme) => {
  ...
  pi.on("input", (event) => {
    relay.onUserInput(event as { text: string; source?: string });
    return undefined;
  });
  ...
  pi.on("model_select", (event) => {
    ...
    if (modelName) relay.onModelSelect(modelName);
  });

  pi.on("thinking_level_select", (event) => {
    const level = event?.level as import("./relay/protocol.js").ThinkingLevel | undefined;
    if (level) relay.onThinkingSelect(level);
  });

  pi.on("agent_start", () => mesh.onAgentStart());

  pi.on("message_start", (event) => relay.onMessageStart(event as { message?: unknown }));

  pi.on("message_update", (event) => {
    relay.onMessageUpdate(event as unknown as { assistantMessageEvent: { type: string; delta?: string } });
  });

  pi.on("tool_execution_start", (event) => {
    relay.onToolExecutionStart(event as { toolCallId: string; toolName: string; args: unknown });
  });

  pi.on("tool_execution_end", (event) => {
    relay.onToolExecutionEnd(event as { toolCallId: string; result: unknown; isError: boolean });
  });

  pi.on("message_end", (event) => relay.onMessageEnd(event as { message?: unknown }));
  ...
  pi.on("session_compact", (event) => relay.onCompact(event as { compactionEntry?: unknown }));
  ...
  pi.on("session_info_changed", (event, ctx) => {
    const newName = (event as { name?: string | undefined }).name ?? undefined;
    ...
    void (async () => {
      ...
      if (wasStarted) relay.stop("peer_stop");
      if (mesh.isJoined) await mesh.rename(requested, cwd);
      if (wasStarted && !disposed) await relay.start(ctx);
    })();
  });
  ...
  pi.on("session_start", (_event, ctx) => {
    lastEventCtx = ctx as typeof lastEventCtx;
    ...
    autoInited = true;
    if (isPrintMode()) return;  // issue #44: never auto-start under -p/--print
    if (!("cwd" in ctx)) return;  // minimal ctx (tests) — nothing to key on
    if (flags.mesh) {
      enableMeshSurface();
      void mesh.join(ctx);
    }
    if (flags.relay) void relay.start(ctx);
  });
  ...
  async function cmdStart(ctx: Pick<ExtensionContext, "ui" | "cwd">, mode?: string): Promise<void> {
    ...
    if (wantRelay && !disposed) await relay.start(ctx);
    cmdStatus(ctx);
  }

  async function cmdStop(ctx: Pick<ExtensionContext, "ui">): Promise<void> {
    ...
    if (relayUp) relay.stop("peer_stop");
    await mesh.leave();
    ctx.ui.notify("[remote-pi] Stopped (mesh + relay disconnected).", "info");
    refreshFooter();
  }

  function cmdStatus(ctx: Pick<ExtensionContext, "ui">): void {
  ...
  pi.registerCommand("remote-pi", {
    ...
    handler: async (args, ctx) => {
      ...
      else if (sub === "devices")                      { await relay.listDevices(ctx); }
      else if (sub.startsWith("revoke"))               { await relay.revoke(sub.slice("revoke".length).trim(), ctx); }
